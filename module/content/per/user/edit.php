...
declare(strict_types=1);
	...
	$qsk = mysqli_query($con,"SELECT * FROM per WHERE id='$folder[3]' AND asal='$_SESSION[ses_user]'");
	$sk=mysqli_fetch_array($qsk);
?>
	<div class='card card-primary'>
		<div class='card-header with-border'>
			<h3 class='card-title'>Edit: <?php echo $sk['judul']; ?></h3>
		</div>

				<div class="card-body">
					<form role="form" enctype="multipart/form-data" method="post" action="/update/per">
							...
							<p>
								...
								<select class='form-control' name='id_jenis' required>
									<option value=''>-- Pilih Jenis--</option>
										<?php
										$qp = mysqli_query($con,"SELECT * FROM kode WHERE kelompok='per' ");
										...
										<select class='select2bs4 select2-primary form-control' multiple='multiple' name='tembusan_struktural[]'>
											<option value=''>-- Pilih Struktural--</option>
												...
												while($s = mysqli_fetch_array($qs)) {
													$cek_sk_struktural=mysqli_num_rows(mysqli_query($con,"SELECT * FROM per WHERE id='$sk[id]' AND tembusan_struktural LIKE '%$s[user]%' "));
										...
										<select class='select2bs4 select2-primary form-control' multiple='multiple' name='tembusan_dosen[]'>
											<option value=''>-- Pilih Dosen--</option>
												...
												while($d = mysqli_fetch_array($qd)) {
													$cek_sk_dosen=mysqli_num_rows(mysqli_query($con,"SELECT * FROM per WHERE id='$sk[id]' AND tembusan_dosen LIKE '%$d[nik]%' "));
										...
										<select class='select2bs4 select2-primary form-control' multiple='multiple' name='tembusan_karyawan[]'>
											<option value=''>-- Pilih Karyawan--</option>
												...
												while($k = mysqli_fetch_array($qk)) {
													$cek_sk_kry=mysqli_num_rows(mysqli_query($con,"SELECT * FROM per WHERE id='$sk[id]' AND tembusan_karyawan LIKE '%$k[nik]%' "));
