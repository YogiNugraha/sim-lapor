...
declare(strict_types=1);
	...
	$qsk = mysqli_query($con,"SELECT * FROM suratkeluar WHERE id='$folder[3]'");
	...
	<div class='card card-primary'>
		<div class='card-header with-border'>
			<h3 class='card-title'>Edit: <?php echo $sk['perihal']; ?></h3>
				...
				<div class="card-body">
					<form role="form" enctype="multipart/form-data" method="post" action="/update/suratkeluar">
						<div class="form-group">
							...
							<p>
								...
								<input name="tujuan" type="text" class="form-control" placeholder="Ditujukan Kepada" value="<?php echo $sk['tujuan']; ?>" required>
							</p>
							...
							<p class="text-center"><b>Share</b></p>
							<hr>
									<p>
										<label>Struktural</label>
										<select class='select2bs4 select2-primary form-control' multiple='multiple' name='tembusan_struktural[]'>
											<option value=''>-- Pilih Struktural--</option>
												<?php
												$qs = mysqli_query($con,"SELECT * FROM level ");
												while($s = mysqli_fetch_array($qs)) {
													$cek_sk_struktural=mysqli_num_rows(mysqli_query($con,"SELECT * FROM suratkeluar WHERE id='$sk[id]' AND tembusan_struktural LIKE '%$s[user]%' "));
									...
									<p>
										<label>Dosen</label>
										<select class='select2bs4 select2-primary form-control' multiple='multiple' name='tembusan_dosen[]'>
											<option value=''>-- Pilih Dosen--</option>
												<?php
												$qd = mysqli_query($con2,"SELECT nik,name,name_glr FROM dosen WHERE status='A' AND nik!='' ORDER BY ikatan,name ");
												while($d = mysqli_fetch_array($qd)) {
													$cek_sk_dosen=mysqli_num_rows(mysqli_query($con,"SELECT * FROM suratkeluar WHERE id='$sk[id]' AND tembusan_dosen LIKE '%$d[nik]%' "));
									...
									<p>
										...
										<select class='select2bs4 select2-primary form-control' multiple='multiple' name='tembusan_karyawan[]'>
											<option value=''>-- Pilih Karyawan--</option>
												<?php
												$qk = mysqli_query($con3,"SELECT name,nik FROM karyawan WHERE status='A' ORDER BY ikatan,name ");
												while($k = mysqli_fetch_array($qk)) {
													$cek_sk_kry=mysqli_num_rows(mysqli_query($con,"SELECT * FROM suratkeluar WHERE id='$sk[id]' AND tembusan_karyawan LIKE '%$k[nik]%' "));
