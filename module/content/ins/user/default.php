...
<?php
	...
	while($p = mysqli_fetch_array($qp)) {
		$jsk = mysqli_num_rows(mysqli_query($con,"SELECT id FROM ins WHERE id_jenis='$p[value]' AND asal='$_SESSION[ses_user]' "));
?>
          ...
          <div class="col-md-12">
            <div class="small-box bg-info">
              ...
              <a href="/ins/jenis/<?php echo $p['value']; ?>" class="small-box-footer">Selengkapnya <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
  		...
  		<div class="card-body">
			<div class='table-responsive'>
				<table class='table table-striped' id='custom'>
					...
					<tbody class='text-sm'>
<?php
	$qsk = mysqli_query($con,"SELECT * FROM ins WHERE tembusan_struktural LIKE '%$_SESSION[ses_user]%' ORDER BY date DESC ");
	while($sk=mysqli_fetch_array($qsk)) {
		$qp = mysqli_query($con,"SELECT `desc` FROM kode WHERE kelompok='ins' AND value='$sk[id_jenis]' ");
